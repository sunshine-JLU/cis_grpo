#!/usr/bin/env bash
# Baseline GRPO on Qwen3-VL-4B-Instruct (non-thinking mode) with ViRL39K,
# 4×RTX-4080-SUPER (32GB).
#
# Same settings as CIS-GRPO v4 fmtonly for apples-to-apples comparison:
#   * /no_think appended to prompts (nothink_dataset.py)
#   * reward_fn_nothink.py — format reward only requires \boxed{...}
#   * Standard RLHFDataset (no counterfactual swapping)
#   * bs=32 matches CIS-GRPO effective batch (16 real + 16 CF)
...
experiment_name=${EXPERIMENT_NAME:-baseline_grpo_qwen3_vl_4b_nothink}
