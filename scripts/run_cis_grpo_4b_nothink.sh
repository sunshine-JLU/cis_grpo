#!/usr/bin/env bash
# CIS-GRPO training on Qwen3-VL-4B-Instruct (non-thinking mode) with ViRL39K,
# 4×RTX-4080-SUPER (32GB).
#
# V4 fmtonly config — same reward structure that beat GRPO on Qwen2.5-VL-3B:
#   * reward_fn_nothink.py — format reward only requires \boxed{...} (no <think>)
#   * Appends /no_think to prompts — suppresses CoT output (cis_dataset.py)
#   * cis_beta=0.0, cis_cf_format_weight=0.1 — CF rows get format-only (0*acc + 0.1*format)
#   * Real rows get standard base reward (0.9*acc + 0.1*format)
#
...
experiment_name=${EXPERIMENT_NAME:-cis_grpo_qwen3_vl_4b_nothink_v4fmtonly}
...
REWARD=(
    ...
    "+custom_reward_function.reward_kwargs.cis_beta=0.0"
    "+custom_reward_function.reward_kwargs.cis_cf_format_weight=0.1"
)
