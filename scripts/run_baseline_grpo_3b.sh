#!/usr/bin/env bash
# Baseline GRPO on Qwen2.5-VL-3B with ViRL39K, on 4xRTX-4080-SUPER (32GB).
# Tuned for 32GB cards: vLLM TP=2 + enforce_eager + param/optim offload + 0.40 mem util.
...
# Keep per-worker thread fanout small so num_proc filter workers don't oversubscribe.
...
experiment_name=${EXPERIMENT_NAME:-baseline_grpo_qwen2_5_vl_3b}

TRAIN_FILES=${TRAIN_FILES:-$PROJECT_ROOT/data/virl39k/train.parquet}
VAL_FILES=${VAL_FILES:-$PROJECT_ROOT/data/virl39k/val.parquet}
