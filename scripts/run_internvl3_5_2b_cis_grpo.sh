#!/usr/bin/env bash
# CIS-GRPO v4 fmtonly on InternVL3.5-2B | ViRL39K | FSDP2 | 4×RTX-4080-SUPER
#
# v4 fmtonly: CF rows get format-only reward (0·acc + 0.1·format), no accuracy
# signal on swapped images. See project_cis_grpo_v4_fmtonly_success.
set -xeuo pipefail
...
CIS_BETA=${CIS_BETA:-0.0}
CIS_CF_FORMAT_WEIGHT=${CIS_CF_FORMAT_WEIGHT:-0.1}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-cis_grpo_internvl3_5_2b_v4fmtonly_$(date +%Y%m%d_%H%M)}
