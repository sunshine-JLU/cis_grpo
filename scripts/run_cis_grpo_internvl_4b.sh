#!/usr/bin/env bash
# CIS-GRPO on InternVL2_5-4B-Instruct (non-thinking) with ViRL39K,
# 4×RTX-4080-SUPER (32GB).
#
# Standard v1 config — cis_beta=0.3, cis_cf_format_weight=0.0.
# reward_fn_nothink.py — format reward only requires \boxed{...}.
# append_no_think=False — InternVL doesn't use Qwen's /no_think convention.
...
MODEL_PATH=${MODEL_PATH:-$PROJECT_ROOT/InternVL2_5-4B}
NNODES=${NNODES:-1}
...
experiment_name=${EXPERIMENT_NAME:-cis_grpo_internvl_4b_v1}
